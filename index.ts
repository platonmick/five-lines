...
class Stone implements Tile {
  ...
  moveHorizontal(dx: number) {
    if (this.falling === FallingState.RESTING) {
...
class Box implements Tile {
  ...
  isAir() { return false; }
  isFallingStone() { return false; }
  isFallingBox() { return this.falling === FallingState.FALLING; }
  isLock1() { return false; }
  isLock2() { return false; }
  draw(g: CanvasRenderingContext2D, x: number, y: number) {
    g.fillStyle = "#8b4513";
...
function updateTile(x: number, y: number) {
  if ((map[y][x].isStony())
    ...
    && map[y + 1][x].isAir()) {
    map[y + 1][x] = new Box(FallingState.FALLING);
    map[y][x] = new Air();
  } else if (map[y][x].isFallingStone()) {
    map[y][x] = new Stone(FallingState.RESTING);
  } else if (map[y][x].isFallingBox()) {
    map[y][x] = new Box(FallingState.RESTING);
  }
}
